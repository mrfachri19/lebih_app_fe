...
function ApprovalOrganisasi() {
  const { id } = useParams();
  const history = useHistory();
  const [dataResto, setDataResto] = useState("");
  function getRestaurant() {
    getAllORganisasi(`/${id}`).then((res) => {
      var tempList;
        ...
        <div className="grid grid-cols-3 gap-20 mx-28 py-10 ">
          <div className="flex">
            ...
            <div className="block ml-10">
              ...
              <p className="font-normal text-sm text-green-20">Type: Organisasi</p>
            </div>
          </div>
          <div>
            <button
...
export default ApprovalOrganisasi;
